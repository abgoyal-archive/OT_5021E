...
#hongmei add MTK_EMMC_OTP_CAMRY for camry&megan
...
$MTK_EMMC_OTP_CAMRY= $ENV{MTK_EMMC_OTP_CAMRY};
...
if($EMMC_SUPPORT eq "yes"){
	...
	if($MTK_EMMC_OTP_SUPPORT eq "yes"){
        ...
        #hongmei change for add camry&megan				
		  if($MTK_EMMC_OTP_CAMRY eq "yes"){	
		       $SHEET_NAME = $SHEET_NAME ." camry" ;
...
# Fill MBR & EBR table -----------------------------------------------------
	for ($iter=0; $iter<@PARTITION_FIELD; $iter++) {   
		...
		if($PARTITION_IDX_FIELD[$iter]>0){
			if($PARTITION_FIELD[$iter] eq "CACHE"||$PARTITION_FIELD[$iter] eq "USRDATA"||$PARTITION_FIELD[$iter] eq "SEC_RO"||$PARTITION_FIELD[$iter] eq "ANDROID"||$PARTITION_FIELD[$iter] eq "CUSTPACK"||$PARTITION_FIELD[$iter] eq "MOBILE_INFO"){
...
sub GenPartSizeFile
{
	...
	for($index=0;$index < $total_rows;$index++){
		if($TYPE_FIELD[$index] eq "EXT4" || $TYPE_FIELD[$index] eq "FAT"){
			...
			if($PARTITION_FIELD[$index] eq "ANDROID" || $PARTITION_FIELD[$index] eq "CACHE" || $PARTITION_FIELD[$index] eq "SEC_RO"|| $PARTITION_FIELD[$index] eq "CUSTPACK" || $PARTITION_FIELD[$index] eq "MOBILE_INFO"){
				$temp -=1;
			}
